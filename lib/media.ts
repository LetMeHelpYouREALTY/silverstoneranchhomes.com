...
export function headingMediaFromText(heading: string): MediaId {
  ...
  if (/(hospital|healthcare|health)/.test(h)) return 'section-hospital'
  if (/(compar|nearby communit|vs\.|versus|peer|competitive edge)/.test(h)) return 'section-comparison'
  ...
  if (/(what.?s included|you.?ll receive|included:)/.test(h)) return 'section-resources'
  if (/(resource|table of contents|directory|next step|deep dive)/.test(h)) return 'section-resources'
  if (/(metric|snapshot|advantage|momentum|appreciation|timing)/.test(h)) return 'section-research'
  if (/(recent sales|sold values|inventory)/.test(h)) return 'hero-listings-kitchen'
  if (/(what we help)/.test(h)) return 'section-buyer-tour'
  ...
  if (/(digital homeowner|homeowner tools|resident portal)/.test(h)) return 'section-hoa-clubhouse'
